...
export default function UtentiRuoliModal({ isOpen, onClose, facilities, isSuperAdmin }) {
  ...
  const inviteUser = async (form) => {
    ...
    try {
      ...
      setInviteResult({ success: true, msg: result.message || `Utente creato. Email inviata a ${form.email}.`, showEmailNote: true });
        ...
        <div className="flex-1 overflow-y-auto bg-white p-8 min-h-0">
          <div className="space-y-5">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <button onClick={() => { setShowNewForm(true); setShowSql(false); setInviteResult(null); }}
            ...
            {inviteResult && (
              <div className={`rounded-2xl p-4 flex items-start gap-3 ${inviteResult.success ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
                ...
                <div>
                  ...
                  {inviteResult.success && inviteResult.showEmailNote && (
              ...
              <div className="space-y-3">
                {filtered.map(u => (
                  <UtenteCard
                    ...
                    onEdit={() => { setInviteResult(null); setEditingId(u.id); }}
...
// ── UtenteCard ────────────────────────────────────────────────
function UtenteCard({ utente: u, facilities, isEditing, isSuperAdmin, onEdit, onCancel, onSaved, onResult, saving, setSaving }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onResult({ success: false, msg: 'Errore: ' + err.message });
    } finally {
      setSaving(false);
    }
  };
